...
- `ignoreWhenInputFocused` = `"auto"` - Whether to ignore the shortcut when the input is focused.
  Defaults to "auto".
...
- `Array<Command>` – A sequence of shortcut commands, the commands have to be pressed in order
  within a short time.
- `Modifier` – A shortcut command that only requires a modifier key to be pressed.

```ts
export type Shortcut = SimpleShortcut | Command | Array<Command> | Modifier;
```
...
- a sequence of keys, e.g. `['g', 'i']` or `['ctrl', 'shift']`
